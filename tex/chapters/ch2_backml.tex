...
Entities $\vect{x}$ and $h(\vect{x})$ can be n-dimensional tensors and encode many
...
(the experience $E$, \aka learning or training set) that it uses to build, optimize
and evaluate the model. The algorithm output is a model $h \in \mathcal{H}$ that
...
the set of all possible color images: $\mathcal{X} \subset \mathbb{N}^{h\times w\times c}$
where $w$, $h$ and $c$ are respectively the image width, height and number of
channels (which equals to 3 in the case of RGB color images). The output space
contains the 3 labels of interest: $\mathcal{Y} = \left\{\textit{human}, \textit{animal}, \textit{object}\right\}$.
...
Some models can also predict structured outputs. This is the case for
\textit{image segmentation} which focuses on classifying each pixel of an image
(\ie what kind of object does this pixel belong to in the image?). The output of
...
objects. This kind of task is called \textit{detection} where the output of the
model, the location, can be encoded as image coordinates $(i, j)$ representing
the object's center of gravity or any of its point. Another common representation
...
In \acrlong{sl}, the training signal is often created by humans anually annotating
examples. Such guidance allows using well-studied methods and usually helps learning
strong models but also comes at a cost of human intervention. This is especially
...
hampers the successful application of \acrlong{sl} but several approaches exist
...
information than the training signal $y$. Coming back to our example of Section
...
Grasping hierarchies is is key for understanding and learning efficiently. As
...
being able to somewhat exploit these inherent hierarchies seems to be relevant to
...
the data and extract meaningful information using field knowledge. This process
...
vision, a great body of work is focused on creating complicated pipelines of
...
trained on the raw images instead \parencite{krizhevsky2012imagenet}. They beat
...
of \acrlong{dl} has created new possibilities for \acrlong{tl}.
...
Sections \ref{ssec:backml:sl} and \ref{ssec:backml:usl} also apply in which case
the supervision qualifier relates to the target task only. For instance, in
...
As an example, let us suppose we would want classify pictures taken with a camera
...
distributions differ is also called \textit{domain adaptation}.
...
to as \textit{feature-based} \acrshort{tl} the case when the knowledge is represented
by the subspace spanned by the features in the source and target domains. The
transferred knowledge can also be embedded as part of the source domain model:
this is \textit{model-based} \acrshort{tl}. Finally, when the knowledge is
transferred as rules specifying the relations between examples in the source
domains, transfer is reffered to as \textit{relation-based}.
...
an explicit memory mechanism to retain such information. These two phenomena are
...
independently. In opposition, it happens that antagonistic tasks cause the resulting
individual task performance to be worse. This issue is related to negative transfer
...
\textit{parameters-based} \acrlong{mtl} use learned models as proxies to extract
...
minimize a risk which can be formalized as the expected risk (for a supervised
problem) \parencite{vapnik1992principles}:
...
error can be studied using the \textit{bias-variance decomposition} of the error
...
the model learned from learning set $\mathcal{S} \in LS$. The \textit{noise} term
is the error of $h_B$ and is therefore irreductible. The \text{bias} is the
difference between $h_B$ and the average model (over models generated from $LS$).
...
the noise in the training data. This phenomenon is called \textit{overfitting}
and hampers these models the ability to generalize, although they typically present
...
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{backml/biasvariancetradeoff.pdf}
...
memorize the training data, the learned function is actually very robust. This
...
expected generalization error used in Section \ref{ssec:backml:bvtradeoff}.
...
whhich differ by how they generate their splits. A common approach is
\textit{$k$-fold \acrlong{cv}} where the data is randomly split into $k$ subsets
and each subset is selected in turn to be the test set (and the remaining folds
make the train set).
...
are trained on the first, selected on the second and the final model is evaluated
on the last. When the dataset is too small for such a three-way split, a common
approach consists in extracting a test set from the initial dataset and then
...
unsuccessful predictions and these different types of elements form what is called
...
probabilities instead of just the class. In this case, one can vary a threshold
...
high variance. The $C$ term in the Lagrange dual formulation is an hyperparameter
...
\begin{figure}
  \centering
  \subfloat[$C=10000$]{\includegraphics[scale=0.6]{backml/svm_c10000.pdf}}
  \subfloat[$C=25$. Notice the new points \\ of class $1$ in the class $0$ cluster.]{\includegraphics[scale=0.6]{backml/svm_c25.pdf}}
...
\end{equation}
which are called \textit{splits}. The leaf nodes of the tree are labelled with
the predictions of the model. In classification, the prediction can either be a
class $\hat{y} \in \mathcal{Y}$ or a probability distribution over $\mathcal{Y}$.
...
based on a supervised dataset. It is a greedy algorithm that, for a node and a
...
\begin{figure}
  \centering
  \subfloat[Fully-developped]{\includegraphics[scale=0.6]{backml/dt_full.pdf}}
  \subfloat[Maximum depth of 3]{\includegraphics[scale=0.6]{backml/dt_max_depth3.pdf}}
...
the problem and reduce the variance, there exists several methods such as pruning
which consists in preventing the final tree to grow too deep. This can be done
during the induction (pre-pruning) or a posteriori (post-pruning). Simple
...
set $S_t$ built by sampling with replacement in the original training set $S$.
...
similarly to the \acrshort{etdic} approach. But rather than using the forest as
a direct classifier, it is used to create a new representation for the image. This
...
$h$ and $\theta^{(l)}$ are the parameters of the function $h_l$ (or layer $l$).
...
task at hand. Although it introduces some complexity when it comes to engineering
the best architecture for a problem, this modularity actually offers a unprecedented
way of implementing inductive bias. Moreover, as long as the chosen functions are
...
\begin{figure}
  \centering
  \includegraphics[scale=0.9]{backml/mlp.pdf}
...
network (\ie a \acrshort{mlp}). Convolutional and pooling layers are introduced
in this section.

\begin{figure}
 \centering
 \includegraphics[scale=0.5]{backml/convnet.png}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.6]{backml/convolutionwithstride.pdf}
...
should ideally decrease when $i$ increases. The parameters update consists in
applying them a correction of amplitude $\gamma$, the learning rate, in opposite
direction of the gradients of $\mathcal{L}$ in the parameters space:
...
Moreover, finding parameters that achieve the global minimum of the training error
...
activations in neural networks. Given a signal $\vect{x} = \left(\vect{x}_1, \vect{x}_2, ..., \vect{x}_m\right)$
(\eg the output of a convolutional layer), the \acrshort{bn} layer maintains an
estimate of the mean $\mu_i$ and variance $\sigma_i$ ($i = 1, ..., m$) of the
preceeding layer output computed over the batch. These estimates are used to
normalize the inputs. Obviously, whether this normalization is actually beneficial
for the network is probably task-, layer- and feature-dependent and there are
...
the input statistics $\mu_i$ and $\sigma_i$ are computed on-the-fly using a moving
average. At inference, the statistics are frozen as well as the learnable parameters
$\alpha_i$ and $\beta_i$.
...
\begin{figure}
  \centering
  \includegraphics[rotate=-90, scale=0.33]{backml/vgg1619.png}
...
together with one connection and one pooling layers (see Figure
...
learnable filter. In order to combine decod high level features and low-level
...
\begin{figure}
  \centering
  \includegraphics[scale=0.55]{backml/transfer-fe.pdf}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.55]{backml/transfer-ft.pdf}
...
The second transfer approach is \textit{fine-tuning} where the task-specific part
...
combine \acrlong{tl} and model compression as the fine-tuning process actually
excludes non-informative feature maps from the final model based on their
\acrfirstit{afds} mechanism.
