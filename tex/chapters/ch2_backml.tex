...
It has the advantage of being simple to interpret, to compute and it applies to both binary and multi-class problems. However, it is affected by \textit{class imbalance}, the situtation when there is a disparity in the numbers of examples belonging to each the problem classes in the dataset. For instance, given binary dataset where $n-1$ examples belong to class $A$ and the last example to class $B$, a classifier predicting only class $A$ would obtain an accuracy close to $1$ which seems very good but in reality the classifier did not learn anything from the data.      
...
An usual way of analysing these metrics is to plot on a two-dimensional graph a model represented as a point $(1 - \textit{specificity}, \textit{sensitivity})$. When the output of a model is the probability of a class, it is possible to generate several pairs of the metrics by varying the threshold applied to the probabilities. Such a plot is called the \acrfirstit{roc} curve (see Figure \TODO{find a figure}). From this plot can be derived an new metric, the \textit{area under the \acrshort{roc} curve} (\rocauc). This metric requires the output of a model to be a ``thresholdable'' number, is more computationally expensive to obtain as one have to evaluate all possible thresholds and only applied to binary classification but provides a measure which is not impacted by class imbalance which is a very interesting property. 
