...
\alpha_k\left(y_k \left(w^T \vect{x}_k + b\right) - 1\right) = 0, \forall k = 1, ..., n
\end{equation}

In Equation \ref{eqn:backml:svmweights}, $\alpha_k > 0$ for the support vectors $x_k$ and $\alpha_k = 0$ for the other samples. This is an interesting property as it implies that only the support vectors and their weights $\alpha_k$ should be stored for future inference. This makes the model very memory efficient for problems where $m \gg n$. This property has also a negative side effect as it means that the model is highly dependent on the support vectors $x_k$. It can therefore change significantly if the $x_k$ are removed or changed which indicates high variance. The $C$ term in the Lagrange dual formulation is an hyperparameter that relaxes the need for linear separability of the training data and allows for points to lie within the gutter or to be misclassified. Low values of $C$ make the optimal solution tolerate more of such points. Reducing the dependency of the model on very few points also decreases the variance but increases the bias. In opposition, high values of $C$ puts a large penalty on points that lie withing the gutter or are misclassified which increases variance but decreases bias. 
...
This section explores tree-based methods. This family of methods was invented at least twice in the 1980s by the \acrshort{ai} \parencite{quinlan1986induction} and the statistics \parencite{breiman2017classification} communities. Section \ref{ssec:backml:dt} presents the basics of decision tree induction, Section \ref{ssec:backml:rf} introduces the random forest algorithms. Section \ref{ssec:backml:et} presents a variant of the random forest called \acrfirstit{et} and Section \ref{ssec:backml:et_image} finally explores how this method can be used for image classification.
...
\end{equation}
and each attribute can be either numerical or categorical (binary or multi-valued). A decision tree is a model structured as a tree of which the nodes $\mathcal{Q}$ are divided in two sets: the internal and leaf nodes $\mathcal{I} \subset \mathcal{Q}$ and $\mathcal{F} \subset \mathcal{Q}$. Each internal node $q \in \mathcal{I}$ tests an input attribute $k_q \in \left\{1,...,m\right\}$ and its $E_q$ outward edges are associated with non-overlapping subsets:
\begin{equation}
\label{eqn:backml:dt:splitsgeneric}
\mathcal{A}^{(q)}_l \subset \mathcal{X}_{k_q},~l = 0,...,E_q-1
\end{equation}
which are called \textit{splits}. The leaf nodes of the tree are labelled with the predictions of the model. In classification, the prediction can either be a class $\hat{y} \in \mathcal{Y}$ or a probability distribution over $\mathcal{Y}$. In regression, the output is a real value $\hat{y} \in \mathcal{Y}$. Given $x_i$, predicting an output value consists in a top-down traversal of the tree. When reaching an internal node $q$ during the traversal, $\vect{x}_i$ will follow the edge $l$ such that $a^{(i)}_{k_q} \in \mathcal{A}^{(q)}_l$ which leads to one of the children nodes of $q$. This process is repeated until a leaf is reached. The prediction and output of the algorithm for $\vect{x}_i$ is the labelling of this leaf. 

In the remainder of the thesis, we will only consider numerical attributes and binary splits at each internal node ($E_q = 2$). These splits are defined by a threshold $v_q$ (or $q.v$) such that:
\begin{align}
\label{eqn:backml:dt:splitsbinary}
\mathcal{A}^{(q)}_0 &= \left]-\infty, v_q\right]\\
\mathcal{A}^{(q)}_1 &= \left]v_q, +\infty\right[
\end{align}
This representation allows for an efficient check at each node as the attribute value has just to be compared to the treshold value. The inference algorithm is formalized in Algorithm \ref{algo:backml:dtinference} for classification with numerical attributes. 

\begin{algorithm}[t]
  \SetAlgoLined
  \KwData{A sample $\vect{x}_i = \left(a^{(i)}_1, a^{(i)}_2, ..., a^{(i)}_m\right)$ and a decision tree $D$.}
  \KwResult{A probability distribution over $\mathcal{Y}$, the prediction for $\vect{x}_i$.}
  \SetKwFunction{dtinfer}{DTInference}
  \Fn{\dtinfer{D, $\vect{x}_i$}}{
    $q = D.root$\;
    \While{$q \not\in \mathcal{F}$}{
      \eIf{$a_{k_q} \leq q.v$}{
        $q = q.\text{left}$\;
  ...
  \caption{Inference with a classification tree $D$ and numerical attributes. The root of the tree is given by $D.root$, $q.left$ and $q.right$ denote the left and right children of a node and the prediction associated with the leaf node $f \in \mathcal{F}$ is accessed by $f.pred$. The regression algorithm only differs by its type of output.}
...
\begin{algorithm}[t]
  \SetAlgoLined
  \SetKwFunction{dtinductrecur}{DTInductionRecursive}
  \SetKwFunction{dtinduct}{DTInduction}
  \SetKwFunction{computepred}{ComputePred}
  \Fn{\dtinduct{$S$}}{
    $D =~$ an empty decision tree\;
    $D.root =~$ \dtinductrecur{$S$}\;
    \KwRet{$D$}\;
  }
  \;
  
  \Fn{\dtinductrecur{$S$}}{
    \If{$I(S) = 0~\vee~$ all attributes in $S$ are constant}{
      $q =~$ a new node\;
      $q.\text{left} =~$ NIL\;
      $q.\text{right} =~$ NIL\;
      $q.\text{pred} =~$ \computepred{$S$}\; 
      \KwRet{$q$}\;
    }
    $k_{best} = -1$\;
    $\Delta I_{best} = 0$\;
    $v_{best} = 0$\;
    \For{$j = 1, ...,|\vect{x}|$}{
      $A =~$ array of examples sorted over values of $j^{th}$ attribute in $S$\;
      \For{$i = 1, ..., |A|-1$}{
        $\Delta I = I(A) - \frac{i}{|A|} I(A[\rightarrow i]) - \frac{|A| - i - 1}{|A|} I(A[(i+1) \rightarrow])$\;
        \If{$\Delta I > \Delta I_{best}$}{
          $k_{best} = j$\;
          $v_{best} = \frac{A[i] + A[i+1]}{2}$\;
          $\Delta I_{best} = \Delta I$\;
        }
      }
    }
    $S_0 = \left\{x_i \mid a^{(i)}_{k_{best}} \leq v_{best} \right\}$\;
    $S_1 = S \setminus S_0$\;
    $q =~$ a new node\;
    $q.v = v_{best}$\;
    $q.\text{left} =~$ \dtinductrecur{$S_0$}\;
    $q.\text{right} =~$ \dtinductrecur{$S_1$}\;
    \KwRet{$q$}\;
  }
...
A tree built using this simple stopping criterion is said to be \textit{fully-developed}. Such a tree is able to capture the training set perfectly which, in practice, hurts generalization as the model clearly overfits. In order to alleviate the problem and reduce the variance, there exists several methods such as pruning which consists in preventing the final tree to grow too deep. This can be done during the induction (pre-pruning) or a posteriori (post-pruning). Simple (pre-)pruning techniques are, for example, stopping the induction when a branch reaches a certain depth, or ensuring that all leaf nodes contain at least a given number of samples. An advantage of decision trees are their interpretability as the rules learned by the model can be easily understood by looking at the tree. In practice, decision trees are rarely used by themselves because of their high variance but rather as part of a forest (see Section \ref{ssec:backml:rf}).
