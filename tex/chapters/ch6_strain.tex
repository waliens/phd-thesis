...
We study five different weighting strategies each producing an intermediate weight $w^{\xi}_{ij}$ where $\xi$ is the strategy identifier. Because we want to avoid the amplitude of the loss and gradients to be impacted by the weighting strategy, we normalize it to obtain the final weight:
\begin{equation}
...
Regarding the ``\textit{constant}'' strategy, we study selected values of $C$ to cover an interval going from $0.01$ to $2$. The specific value $C = 1$ corresponds to to the absence of weights in the loss. We believe that $C < 1$ should be preferrable in higher scarcity regimes as pseudo-labels are likely to impact negatively the training process so it makes sense to tune down their contributions to the loss. 
...
\begin{table}
  ...
  \begin{tabular}{|cc|c|}
    \hline
    \multicolumn{2}{|c|}{$|\mathcal{D}|_s = 0$} & $80.88 \pm 0.37$ \\
    \multicolumn{2}{|c|}{$\mathcal{D}_l$ only}  & $80.14 \pm 1.06$ \\
    \multicolumn{2}{|c|}{$\mathcal{D}_l \cup \mathcal{D}_s$} & $74.85 \pm 2.53$ \\
    \hline
    constant & $C=0.5$ & $81.22 \pm 0.40$ \\
    \hline
  \end{tabular}
...
\TODO{New scores see Table \ref{tab:strain:thyroidresults}}
...
\begin{table}
  ...
  \begin{tabular}{|c|c|}
    ...
    Method & Dice \\
    \hline
    Self-training & $82.77 \pm 1.45$ \\
    $\mathcal{D}_l$ only & $75.36 \pm 4.49$ \\  
    $\mathcal{D}_l \cup \mathcal{D}_s$ & $78.24 \pm 3.09$ \\
    \hline
  \end{tabular}
  \caption{Experiment on the Thyroid \acrshort{fnab}. Dice evaluated on test+val, threshold tuned on $\mathcal{D}_l$.}
