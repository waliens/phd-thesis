...
Related self-training works can be found in Sections \ref{ssec:backml:dl:selftraining} and \ref{ssec:backdp:st}. Beyond self-training, we would also like to point out that there exist methods, implemented in software such as QuPath \cite{bankhead2017qupath}, Ilastik \cite{berg2019} or Cytomine \cite{maree2016collaborative} and based on traditional computer vision or machine learning, that allow users to interactively complete a partial hand-drawn annotation of a given image. Among these methods are, for instance, Graph Cut \cite{kwatra2003graphcut} and GrabCut \cite{rother2004grabcut}, or more recently DEXTRE \cite{maninis2018deep} and NuClick \cite{koohbanani2020nuclick}. While the self-training approach explored in this chapter can certainly be exploited in an interactive mode, this question will be left as future work.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{strain/method-diagram.pdf}
  \caption{Our approach illustrated. The model is first warmed-up on the exhaustively-labeled data then further traind by self-training on the combined sparsely- and exhaustively-labeled sets. A more formal definition is provided in Algorithm \ref{algo:strain:selftraining}.}
